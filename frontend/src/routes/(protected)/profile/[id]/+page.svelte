...
import { user, isAuthenticated, logout, getProfile, updateProfile } from '$lib/services/authService';
...
const loadProfile = async () => {
  try {
    ...
    // Load current user's profile (no ID parameter)
    profileData = await getProfile();
    aboutMe = profileData.user.about_me || "";
  } catch (err) {
...
const handleUpdateProfile = async () => {
  try {
    await updateProfile(aboutMe);
    editing = false;
    // Profile data is automatically refreshed by updateProfile
  } catch (err) {
...
onMount(() => {
  // Check authentication first, then load profile
  const unsubscribe = isAuthenticated.subscribe(authenticated => {
    if (authenticated === false) {
      goto('/login');
    } else {
      // Always try to load profile - getProfile will handle auth state
      loadProfile();
    }
  });
        ...
        <div class="profile-content">
          <div class="profile-header">
            ...
            <div class="profile-info">
              ...
              {#if editing}
                <textarea bind:value={aboutMe} class="bio-textarea"></textarea>
                <div class="button-group">
                  <button class="create-post-btn" on:click={updateProfile}>Save</button>
                  <button class="logout-btn" on:click={() => (editing = false)}>Cancel</button>
                </div>
              {:else}
                {#if profileData.user?.about_me}
                  <p class="bio">{profileData.user.about_me}</p>
                {/if}
                <button class="create-post-btn" on:click={() => (editing = true)}>Edit Bio</button>
              {/if}
            </div>
